...
`SeqStream` is a sequential, pull-based implementation of `Stream`.
...
`SpliteratorSeqStream.spliterator()` is the single acquisition point. It
rejects a consumed stage or a stage whose shared pipeline is closed, then
returns and clears the stage's spliterator. A spliterator already returned to
...
Intermediate operations construct a stream over their new spliterator and the
existing pipeline with `viewOf(spliterator, pipeline())`. The spliterators
...
mode survives conversion to and from `Stream`. The spliterators are
...
Because traversal is pull-based, an inner stream that produced the last
requested element remains open until the next traversal attempt observes its
exhaustion or until the pipeline is closed. Short-circuiting and traversal
failure are therefore partial traversal: callers must close the returned
stream to guarantee that the current inner stream is closed. This differs from
JDK `flatMap`, but allows an inner stream to be infinite.
...
is first advanced. `Split.defer` is therefore the mechanism for `sorted` and
