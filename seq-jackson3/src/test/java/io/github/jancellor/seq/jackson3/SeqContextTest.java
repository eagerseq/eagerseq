...
public class SeqContextTest {
    ...
    public void malformedArrayFails() throws Exception {
        assertThrows(JacksonException.class, () -> mapper
                .readValue("[\"a\",", new TypeReference<Seq<String>>() {
                }));
    }
    @Test
    ...
    @Test
    public void moduleAdvertisesOnlySeqDeserialization() {
        AtomicReference<Deserializers> registered = new AtomicReference<>();
        SetupContext context = (SetupContext) Proxy
                .newProxyInstance(getClass().getClassLoader(),
                        new Class<?>[]{
                                SetupContext.class},
                        (proxy, method, arguments) -> {
                            if (!method.getName().equals("addDeserializers")) {
                                throw new AssertionError(method.getName());
                            }
                            registered.set(
                                    (Deserializers) arguments[0]);
                            return null;
                        });
        new SeqModule().setupModule(context);
        assertThat(
                registered.get().hasDeserializerFor(
                        mapper.deserializationConfig(), Seq.class),
                is(true));
        assertThat(registered.get().hasDeserializerFor(
                mapper.deserializationConfig(), List.class),
                is(false));
    }
    ...
    public static class InspectingElement extends ValueDeserializer<String> {
    ...
    public static class Upper extends ValueDeserializer<String> {

        @Override
        public String deserialize(JsonParser parser,
                DeserializationContext context)
                throws JacksonException {
            return parser.getString().toUpperCase(Locale.ROOT);
        }
    }
